...
// .wrangler/tmp/pages-05JLPe/functionsWorker-0.18386145158512945.mjs
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        // don’t reflect huge downstream responses
      }),
...
async function onRequestPost3({ request, env }) {
  try {
    ...
    const smsMode = String(env.SMS_MODE || "off").toLowerCase().trim();
    ...
    const allowed = lineType === "mobile" || lineType === "landline" || lineType === "fixed_voip";
    if (!allowed) {
      return new Response(
        JSON.stringify({ ok: false, error: "Invalid phone number." }),
        { status: 400, headers: { "Content-Type": "application/json" } }
    ...
    const shouldSendSms = smsMode === "on" && !isDryRun && lineType === "mobile";
    if (!shouldSendSms) {
      return new Response(
        JSON.stringify({
          ok: true,
...
// .wrangler/tmp/bundle-SaxRnF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SaxRnF/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.18386145158512945.js.map
